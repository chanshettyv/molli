...
async def test_escalates_hostile_work_environment():
    verdict = await guardrail.check("my manager is creating a hostile work environment", USER)
    assert verdict.action == Action.ESCALATE
...
async def test_escalates_age_discrimination():
    verdict = await guardrail.check("I was passed over for a promotion because of my age", USER)
    assert verdict.action == Action.ESCALATE
...
async def test_escalates_retaliation():
    verdict = await guardrail.check("I am scared I will face retaliation for reporting this", USER)
    assert verdict.action == Action.ESCALATE
...
async def test_escalates_threatened_for_reporting():
    verdict = await guardrail.check("my manager threatened me for reporting the incident", USER)
    assert verdict.action == Action.ESCALATE
...
async def test_canned_response_does_not_mention_manager():
    verdict = await guardrail.check("I am being harassed at work", USER)
    assert "direct manager" not in verdict.canned_response.lower() or "not" in verdict.canned_response.lower()
...
async def test_allows_discrimination_training():
    verdict = await guardrail.check("how do I complete the discrimination training module?", USER)
    assert verdict.action == Action.ALLOW
